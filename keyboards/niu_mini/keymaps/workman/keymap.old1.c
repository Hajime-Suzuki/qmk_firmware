...
[_BL] = KEYMAP(
  ...
  // KC_ENT,  KC_LGUI, KC_LCMD, LT(_SHORTCUTS, KC_ESC), LT(_FL1, KC_SPC), LT(_SHORTCUTS, KC_TAB),           OSM(MOD_LSFT), LT(_MOUSE, KC_BSPC), LT(_NUM, KC_DEL),KC_DOWN, MO(_LED_SOUND), KC_BSPC
),

// Original
// [_BL] = KEYMAP(
//   KC_LALT          , KC_Q, KC_D, KC_R, KC_W, KC_B,       KC_J, KC_F, KC_U   , KC_P  , S(LCMD(KC_F8)) , OSM(MOD_LGUI | MOD_LSFT),
//   OSM(MOD_LGUI)    , KC_A, KC_S, KC_H, KC_T, KC_G,       KC_Y, KC_N, KC_E   , KC_O  , KC_I       , KC_MINS,
//   LT(_LEFT, KC_ESC), KC_Z, KC_V, KC_M, KC_C, KC_X,       KC_K, KC_L, KC_COMM, KC_DOT, KC_SLSH    , KC_SCLN ,
//
//   KC_ENT,  MO(_LEFT_NUM), MO(_LEFT2), LT(_PROGRAMMING, KC_TAB), LT(_SHORTCUTS, KC_SPC), MO(_FL1),        OSM(MOD_LSFT), LT(_NUM, KC_BSPC), LT(_MOUSE, KC_DEL), KC_LGUI, MO(_LED_SOUND), S(LCMD(KC_F9))
//   // KC_ENT,  KC_LGUI, KC_LCMD, LT(_SHORTCUTS, KC_ESC), LT(_FL1, KC_SPC), LT(_SHORTCUTS, KC_TAB),           OSM(MOD_LSFT), LT(_MOUSE, KC_BSPC), LT(_NUM, KC_DEL),KC_DOWN, MO(_LED_SOUND), KC_BSPC
// ),
...
[_FL1] = KEYMAP(
  ...
  KC_PIPE, KC_DLR     , KC_LPRN, KC_RPRN     , TD(TD_COL_SEMC), KC_EXLM,        LCMD(KC_LEFT), KC_LEFT          , KC_DOWN    , KC_RGHT         , LCMD(KC_RGHT), xxxxx,
  KC_BSLS, TD(GT_RBRC), KC_LBRC, KC_EQL      , TD(QUO_DQUO)   , KC_QUES,        KC_COMM      , LALT(KC_LEFT)    , LCMD(KC_UP), LALT(KC_RGHT)   , LCMD(KC_DOWN), KC_LSFT,
...
// [_FL1] = KEYMAP(
//   KC_AMPR, KC_SCLN   , TD(LCBR_AT)  , TD(RCBR_HASH), TD(COPY_CUT), _PASTE     ,        xxxxx   , S(LCMD(KC_LEFT)) , KC_UP      , S(LCMD(KC_RGHT)), KC_LCMD      , KC_LALT,
//   KC_PIPE, TD(DOL_LT), TD(LPRN_TILD), TD(RPRN_CIRC), TD(CLN_UNDR), KC_EXLM    ,        KC_HOME , KC_LEFT          , KC_DOWN    , KC_RGHT         , KC_END       , _______,
//   KC_BSLS, KC_GT     , KC_LBRC      , TD(EQL_RBRC) , TD(QUO_DQUO), TD(QES_GRV),        KC_COMM , LCMD(KC_LEFT)    , LCMD(KC_UP), LCMD(KC_RGHT)   , LCMD(KC_DOWN), KC_LSFT,
//   _______, _______   , _______      , KC_TAB       , S(KC_TAB)   , _______    ,        MO(_FL2), LT(_NUM, KC_BSPC), KC_SPC     , _______         , _______      , _______
// ),
...
//Mac
[_SHORTCUTS] = KEYMAP(
  LCTL(KC_F1), LCTL(KC_F2), LCTL(KC_F3), LCTL(KC_F4), LCTL(KC_F5) , xxxxx  ,                _______,     _______,     _______,    _______,      _______,      _______,
  LCTL(KC_F6), _SAVE      , _S_NO_FRMT , _UNDO      , _REDO       , xxxxx  ,                _______,     _______,     _______,    _______,      _______,      _______,
  LCTL(KC_F7), LCMD(KC_W) , LCTL(KC_F8), LCTL(KC_F9), LCTL(KC_F10), xxxxx  ,                _______,     _______,     _______,    _______,      _______,      _______,
  _______    , _______    ,  _______   , _______    , _______     , _______,                _______,     _______,     _______,    _______,      _______,      _______
),

// [_FL2] = KEYMAP(
//   _______,     _______ ,   _______,   _______,   _______,    _______,         xxxxx           , xxxxx     , S(KC_UP)  , xxxxx           , xxxxx     , _______,
//   _______,     KC_TILDE,   KC_GRV ,   KC_AT  ,   KC_SCLN,    KC_HASH,         S(LCMD(KC_LEFT)), S(KC_LEFT), S(KC_DOWN), S(KC_RGHT), S(LCMD(KC_RGHT)) , _______,
//   _______,     KC_LT   ,   KC_CIRC,   KC_RBRC,   KC_DQT ,    _______,         xxxxx           , _block_s_u, _block_u  , _block_s_d      , _block_d  , _______,
//   _______,     _______ ,   _______,   _______,   _______,    _______,         _______         , _______   , _______   , _______         , _______  , _______
// ),
...
// [_FL1] = KEYMAP(
//   _______, TD(TD_BCSL_TILD), KC_LCBR        , KC_RCBR         , _COPY           , _PASTE  ,         _CUT   , S(LCMD(KC_LEFT)) , KC_UP      , S(LCMD(KC_RGHT)), _______      , _______,
//   _______, TD(TD_DOL_UNDR) , TD(TD_L_BRCT)  , TD(TD_R_BRCT)   , TD(TD_COL_SEMC) , KC_EXLM,          KC_HOME, KC_LEFT          , KC_DOWN    , KC_RGHT         , KC_END       , _______,
//   _______, TD(TD_AMP_PIP)  , TD(TD_HS_AT_CI), TD(TD_EDL_GT_LT), TD(TD_QUOTATION), KC_QUES,          KC_COMM, LCMD(KC_LEFT)    , LCMD(KC_UP), LCMD(KC_RGHT)   , LCMD(KC_DOWN), _______,
...
[_PROGRAMMING] = KEYMAP(
  S(LCTL(KC_F1))       , S(LCTL(KC_F2))  , S(LCTL(KC_F3)) , S(LCTL(KC_F4)),S(LCTL(KC_F5)) ,S(LCTL(KC_F6)),        _______,     _______      , KC_UP  , _______      , _______,  _terminal,
  S(LALT(LCMD(KC_F11))), S(LCMD(KC_F11)) , S(LCMD(KC_F10)), _Dec1         , _Inc1         ,S(LCTL(KC_F7)),        KC_HOME,     KC_LEFT      , KC_DOWN, KC_RGHT      , KC_END ,  _t_focus,
  S(LALT(LCMD(KC_F12))), LCMD(LALT(KC_C)), LCMD(KC_L)     , _DupLine      , LCMD(KC_SLASH),S(LCTL(KC_F8)) ,        _______,     LCMD(KC_LEFT), _______, LCMD(KC_RGHT), _______,  _______,
  _______              , _______         , _______        , _______       , _______       , _______,        _______,     S(LCMD(KC_J)), _______, _______      , _______,  _______
),


[_LEFT] = KEYMAP(
  xxxxx  ,     xxxxx          , xxxxx        , xxxxx        ,      xxxxx  ,      xxxxx  ,                _______,     _______      , LCTL(KC_UP)  , _______      , _______, _______,
  xxxxx  ,     xxxxx          , LCTL(KC_LEFT), LCTL(KC_RGHT),      xxxxx  ,      xxxxx  ,                _______,     LCTL(KC_LEFT), LCTL(KC_DOWN), LCTL(KC_RGHT), _______, _______,
  xxxxx  ,     S(LSFT(KC_F12)), KC_LCMD      , KC_TAB       ,      xxxxx  ,      xxxxx  ,                _______,     _______      , _______      , _______      , _______, _______,
  _______,     _______        , _______      , _______      ,      _______,      _______,                _______,     _______      , _______      , _______      , _______, _______
),
...
//   _______,     LCMD(KC_W),  _______,     S(LCMD(KC_TAB)), LCMD(KC_TAB), _______,                _______,     _______,     _______,    _______,      _______,      _______,
//   _______,     _______   ,  _______,     _______        , _______     , _______,                _______,     _______,     _______,    _______,      _______,      _______
// ),
